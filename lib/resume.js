...
function experienceFit(job, profile) {
  ...
  if (/(director|head of|vice president|\bvp\b|principal)/.test(t))
    return { level: 'reach', label: 'leadership role — reach', score: 2 };
  if (/(manager|team lead|\blead\b)/.test(t))
    return { level: 'reach', label: 'manager/lead — reach at your level', score: 5 };

  const nums = ((job.experience || '').match(/\d+/g) || []).map(Number);
  const reqMin = nums.length ? Math.min(...nums) : null;
  if (reqMin != null) {
    if (reqMin <= years + 1) return { level: 'good', label: `needs ${job.experience} — you fit`, score: 25 };
    if (reqMin <= years + 3) return { level: 'stretch', label: `needs ${job.experience} — a stretch`, score: 12 };
    return { level: 'reach', label: `needs ${job.experience} — reach`, score: 3 };
  }
  if (/senior|sr\.?|consultant/.test(t)) return { level: 'stretch', label: 'senior title — stretch', score: 12 };
  return { level: 'good', label: 'level looks right', score: 22 };
}
