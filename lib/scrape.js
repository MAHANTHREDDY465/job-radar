...
// When indiaOnly is set, keep India + remote + unknown-location (unknown gets
// re-checked after the detail fetch fills in the real location).
function locationOk(job, profile) {
  if (!profile.indiaOnly) return true;
  const loc = job.location || '';
  if (!loc) return true;
  return salary.isIndia(loc) || salary.isRemote(loc);
}
...
async function runScrape({ log = console.log } = {}) {
  ...
  for (const c of (companies.workday || []).filter((c) => c.enabled !== false)) {
    log(`• Workday: ${c.name}`);
    try {
      const listed = await workday.fetchWorkday(c, profile.searchTerms || profile.titleKeywords);
      const relevant = listed.filter((j) => keep(j, profile) && locationOk(j, profile)).slice(0, 40);
      ...
      for (const job of relevant) {
        ...
        if (locationOk(job, profile)) collected.push(job); // drop non-India once real location is known
      }
    } catch (e) {
  ...
  for (const [key, fn] of simple) {
    for (const c of (companies[key] || []).filter((c) => c.enabled !== false)) {
      ...
      try {
        ...
        const relevant = listed.filter((j) => keep(j, profile) && locationOk(j, profile));
        log(`    ${listed.length} postings, ${relevant.length} relevant`);
